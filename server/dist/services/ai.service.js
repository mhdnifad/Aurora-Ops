...
class AIService {
    ...
    constructor() {
        ...
        if (apiKey && apiKey !== 'your_openai_api_key') {
            ...
            this.enabled = true;
            logger_1.default.info('✅ OpenAI Service initialized');
        }
        else {
            logger_1.default.warn('⚠️  OpenAI API key not configured - AI features disabled');
        }
    }
    /**
     ...
     */
    async generateTaskSummary(taskDescription, taskTitle) {
        ...
        try {
            ...
            logger_1.default.error('Error generating task summary:', error);
...
- Avg Task Completion: ${data.avgTaskCompletionDays} days`,
            ...
            logger_1.default.error('Error generating business insights:', error);
    ...
    async analyzeTeamPerformance(teamData) {
        ...
        try {
            const memberSummary = teamData.members.map(m => `${m.name}: ${m.tasksCompleted} tasks, ${m.avgCompletionTime}d avg, ${m.overdueCount} overdue`).join('\n');
            const response = await this.openai.chat.completions.create({
                model: 'gpt-3.5-turbo',
                messages: [
                    {
            ...
            logger_1.default.error('Error analyzing team performance:', error);
...
${projectData.deadline ? `Deadline: ${projectData.deadline.toLocaleDateString()}` : 'No deadline set'}`,
            ...
            logger_1.default.error('Error generating project recommendations:', error);
    ...
    async generateTaskTitle(description) {
        ...
        try {
            ...
            logger_1.default.error('Error generating task title:', error);
